...
-- New: Option to auto-select source based on mouse position
local auto_select_source_by_mouse = false

-- Ensure set_crop_settings is defined early so it is always available
...
-- Helper: Get all display capture sources in the current scene and their monitor geometries
function get_all_display_sources_with_geometry()
    local sources_with_geom = {}
    local sources = obs.obs_enum_sources()
    if sources ~= nil then
        for _, src in ipairs(sources) do
            local src_id = obs.obs_source_get_id(src)
            if src_id == get_dc_info().source_id then
                local name = obs.obs_source_get_name(src)
                local geom = get_monitor_info(src)
                if geom and geom.x and geom.y and geom.width and geom.height then
                    table.insert(sources_with_geom, {
                        name = name,
                        source = src,
                        geom = geom
                    })
    ...
    return sources_with_geom
end

-- Helper: Find which monitor contains the mouse position
function find_monitor_for_mouse(mouse)
    local sources = get_all_display_sources_with_geometry()
    for _, entry in ipairs(sources) do
        local g = entry.geom
        if mouse.x >= g.x and mouse.x < g.x + g.width and mouse.y >= g.y and mouse.y < g.y + g.height then
            return entry.name, entry.source, g
        end
    end
    return nil, nil, nil
end

-- Patch: Before zoom/follow, auto-select the correct source if enabled
local function auto_select_source_if_enabled()
    if auto_select_source_by_mouse then
...
-- Patch into on_toggle_zoom and on_timer
function on_toggle_zoom(pressed)
    if pressed then
        auto_select_source_if_enabled()
...
-- Add to script_properties and script_update
function script_properties()
    ...
    -- Add the new auto-select property
    local auto_select = obs.obs_properties_add_bool(props, "auto_select_source_by_mouse", "Auto-select display source by mouse position")
    obs.obs_property_set_long_description(auto_select, "When enabled, the script will automatically select the display capture source for the monitor under the mouse before zooming or following.")

    return props
end

function script_update(settings)
    auto_select_source_by_mouse = obs.obs_data_get_bool(settings, "auto_select_source_by_mouse")
                ...
                if monitor_id_prop then
                    ...
                    if settings ~= nil then
                        ...
                        for i = 0, item_count do
                            ...
                            -- Log all available monitor property names and values for debugging
                            log(string.format("Monitor property [%d]: name='%s', value='%s'", i, tostring(name), tostring(value)))
                    ...
                    -- Try to parse the monitor geometry from the found string
                    if found then
                        ...
                        log("WARNING: Could not parse monitor geometry from display name.\n" ..
                            "         Please check the above log for available monitor property names and values.\n" ..
                            "         If the script cannot auto-detect, use the 'Set manual source position' option and add override values.")
                    end
                end
